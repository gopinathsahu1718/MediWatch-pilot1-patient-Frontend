...
export default function HomePage() {
  ...
  const completedDays = Math.max(
    ...
    dashboard?.todaySubmitted
      ...
      : (dashboard?.currentDay || 1) - 1,
  );

  const progress = (completedDays / (dashboard?.totalDays || 1)) * 100;
  ...
  `}
        ...
        `}
                    ...
                    <div className="text-right">
                      {isMissed ? (
                        <>
                          <p className="text-[10px] text-orange-500">Status</p>

                          <h4 className="text-xs font-bold text-orange-500">
                            Missed
                          </h4>
                        </>
                      ) : (
